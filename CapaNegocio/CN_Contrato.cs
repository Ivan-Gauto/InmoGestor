...
namespace CapaNegocio
{
    public class CN_Contrato
    {
        ...
        public List<ContratoAlquiler> ConsultarContratos(
            int? estado = null, int? inmuebleId = null, string dniInquilino = null,
            DateTime? desde = null, DateTime? hasta = null)
        {
            return _cdContrato.ListarContratos(estado, inmuebleId, dniInquilino, desde, hasta);
        }

        public ContratoAlquiler ObtenerDetalleContrato(int contratoId)
        {
            return _cdContrato.ObtenerDetalle(contratoId);
        }

        public bool FinalizarContratoSiCorresponde(int contratoId, out string mensaje)
        {
            // Podrías agregar validaciones previas (p.ej., no finalizado/anulado)
            return _cdContrato.FinalizarSiTodasPagadas(contratoId, out mensaje);
        }
                                            ...
                                            string descAdicionales = "")
        {
            ...
            {
                var periodoDate = fechaInicio.AddMonths(i);
                var fechaVenc = CalcularVencimiento(periodoDate.Year, periodoDate.Month, diaVenc);
                var periodo = FormatearPeriodo(periodoDate); // "AAAAMM"

                var q = new Cuota
                {
                    // ContratoId se setea después de insertar el contrato
                    NroCuota = i + 1,
                    Periodo = periodo,
                    FechaVenc = fechaVenc,
        ...
        public bool RegistrarContrato(ContratoAlquiler contrato, out int contratoId, out string mensaje)
        {
            ...
            if (contrato.FechaFin == default(DateTime))
            {
                ...
                var fin = contrato.FechaInicio.AddMonths(contrato.CantidadCuotas);
            ...
            if (contrato.Estado == 0) contrato.Estado = 1; // 1 = Vigente

            // Validaciones de negocio
            var error = ValidarContrato(contrato);
            ...
            return _cdContrato.RegistrarContratoYCuotas(contrato, cuotas, out contratoId, out mensaje);
        }
        ...
        {
            return _cdContrato.AnularContrato(contratoId, out mensaje);
        }
